...
try:
    ...
    print(
        "Attempting to find json module in:",
        [p for p in sys.path if "json" in str(p).lower()],
    )
    sys.exit(1)
...
class NSRLConverter:
    ...
    def _create_constant_objects(self) -> Dict[str, Any]:
        """Create constant UCO objects (Tool, Organization, Source)."""
        current_time = datetime.datetime.now(timezone.utc).strftime(
            "%Y-%m-%dT%H:%M:%S.%fZ"
        )
    ...
    def _create_bundle(self) -> Dict[str, Any]:
    ...
    def _create_environment_object(self) -> Dict[str, Any]:
    ...
    def _create_file_facet(
        self, media_file: MediaFile, facet_id: str
    ) -> Dict[str, Any]:
        ...
        facet = {
            ...
            "uco-observable:extension": (
                os.path.splitext(media_file["FileName"])[1][1:]
    ...
    def _create_content_data_facet(
        self,
    ...
    def process_file(self, input_file: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            objects: List[Dict[str, Any]] = []  # List to store all objects
            added_ids: Set[str] = set()  # Track added object IDs
            ...
            for media in media_list:
                ...
                for media_file in media.get("MediaFiles", []):
                    ...
                    safe_name = hashlib.md5(
                        media_file.get("FileName", "").encode()
                    ).hexdigest()
                    ...
                    cdf_id = self.create_identifier(
                        "facet-content", f"{media_id}-{safe_name}"
                    )
                    cdf = self._create_content_data_facet(
                        media_file, media, cdf_id, hash_refs
                    )

                    # Create FileFacet (no hashes)
                    ff_id = self.create_identifier(
                        "facet-file", f"{media_id}-{safe_name}"
                    )
    ...
    def process_input(self) -> None:
        ...
        if self.combine and combined_results:
            combined = {"@context": UCO_CONTEXT["@context"], "@graph": combined_results}
...
def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Convert NSRL CAID JSON to UCO format")
    parser.add_argument("input", help="Input JSON file or directory")
    parser.add_argument(
        "-o", "--output", default="output", help="Output directory (default: output)"
    )
    parser.add_argument(
        "--combine", action="store_true", help="Combine all output into single graph"
    )
    parser.add_argument(
        "--validate", action="store_true", help="Validate output against UCO schema"
    )
